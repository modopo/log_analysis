...
def connect():
    ...
    try:
        ...
        print("Connection unsuccessful: " + e.pgerror)
...
def get_error_rate():
    ...
    result = process("""select to_char(failed.date, 'Mon DD, YYYY'), 
                            failed.fcount, success.scount
                            from (select time::date as date, 
                            count(status) as fcount
                            from log
                            where status != '200 OK'
    ...
    print("\n\nThese are the day(s) that "
          "have more than 1% of requests lead to error: \n")
    for x in result:
